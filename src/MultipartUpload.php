...
use Sunsgne\WebmanMultipartUpload\exception\UploadException;
use Webman\File;
/**
 * @Time 2022/12/26 14:51
 * @author sunsgne
 */
class MultipartUpload
{
     ...
     */
    public function setConfig(array|string $config, mixed $value = null): MultipartUpload
    {
        if (is_array($config)) {
            $this->config = array_merge($this->config, $config);
        } else {
     ...
     * @param File $file 文件流对象
     * @param integer $chunk 文件分片序号，从0递增到N
     * @throws UploadException
     * @return false|array 文件保存路径
     */
    public function upload(string $fileID, File $file, int $chunk = 0): array|bool
    {
        // 检测上传保存路径
        if (!$this->checkPath()) {
            return false;
        ...
        if (!FileHandle::instance()->createDir($tmpPath)) {
    ...
    public function merge(string $fileID, int $chunkLength, string $fileName, string $saveDir = ''): array
    {
        ...
        // 验证文件名
        $ext = FileHandle::instance()->getExt($fileName);
        ...
        if (!empty($saveDir) && !is_dir($savePath)) {
            if (!FileHandle::instance()->createDir($savePath)) {
        ...
        for ($k = 0; $k < $chunkLength; $k++) {
            ...
            unset($readerFp);
            // 删除临时文件
            FileHandle::instance()->removeFile($chunkPath);
        }
        // 关闭保存文件句柄
        fclose($writerFp);
        // 删除临时目录
        FileHandle::instance()->removeDir($tmpPath);
    ...
    protected function checkPath(): bool
    {
        ...
        if ((!is_dir($rootPath) && !FileHandle::instance()->createDir($rootPath)) || (is_dir($rootPath) && !is_writable($rootPath))) {
        ...
        if ((!is_dir($tmpPath) && !FileHandle::instance()->createDir($tmpPath)) || (is_dir($tmpPath) && !is_writable($tmpPath))) {
